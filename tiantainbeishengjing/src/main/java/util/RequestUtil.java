...
public class RequestUtil {
	...
	// 转换查询条件时间
	public static void transSearchDate(Map paramMap) throws Exception {
		String TIMETYPE = (String) paramMap.get("date_format");
		String STARTDATE = (String) paramMap.get("start_date");
		String ENDDATE = (String) paramMap.get("end_date");
		if (!StringUtil.isEmpty(TIMETYPE) && !StringUtil.isEmpty(STARTDATE) && !StringUtil.isEmpty(ENDDATE)) {
			String formart = "";
			switch (TIMETYPE) {
			case "HOURLY":
			...
			try {
				...
				if (temp >= 0) {
					...
					case "HOURLY":
						 ...
						 * Exception(); else { cal.setTime(END); END =
						 * cal.getTime(); }
						 */
						break;
					case "DAILY":
						 ...
						 * Exception(); else { cal.setTime(END);
					...
					case "MONTHLY":
						 ...
						 * END.getTime(); if(months < 0) throw new Exception();
					...
					throw new Exception("开始和结束日期不可相同");
				}
				paramMap.put("start_date", df.format(START));
				paramMap.put("end_date", df.format(END));
			} catch (Exception e) {
